...
import { capitalize, formatString } from '../../utils/capitalize.js';
...
class CalendarElement extends LitElement {
  static properties = {
    ...
    isOpen: { type: Boolean },
    label: { type: String },
    dayOfWeekForFirstDate: { type: Number },
    skipNumberOfDays: { type: Array },
    months: { type: Array },
    arrayOfYearsOptions: { type: Array },
  };
...
`;
  ...
  populateDays() {
    ...
    const firstDate = new Date(this.currentYear, this.currentMonth, 1, 0).getDay();
    this.skipNumberOfDays = firstDate === 0 ? [] : Array.from({ length: firstDate });
  }
  ...
  yearsOptions() {
    ...
    const arrayOfYearsOptions = Array.from({ length: lastYear - firstYear }, (_, i) =>  i + firstYear);
